              ...
              <Grid item xs={12} style={{ display: "flex", alignItems: "center" }} className={classes.alignval}>
                {props.price ? (
                  <Typography style={{ display: "flex", width: "100%", padding: 0 }} className={classes.resetpadd}>
                    <Typography
                      style={{ fontSize: "22px" }}
                      className={`${(props.price != null) & (props.price !== "") ? "" : "shine"} ${classes.colorMain} ${
                        classes.h6FontSize
                      } ${classes.offerPricePadding} ${isSilver ? classes.pricesilver : ""}`}
                    >
                ...
                {props.price ? (
                  <Typography style={{ display: "flex", width: "100%" }} className={classes.resetpadd}>
                    <Typography
                      ...
                      className={`pricing-p${(props.price != null) & (props.price !== "") ? "" : "shine"} ${
                        classes.deletePrice
                      } ${classes.dis}`}
                    >
                      {props.offerPrice === props.price ? "" : <del>₹&nbsp;{Math.round(product_quantity * props.price)}</del>}
            ...
            >
              {props.price ? (
                <Typography style={{ display: "flex", width: "100%" }}>
                  <Typography
                    // variant="caption"
                    className={`pricing-p${(props.price != null) & (props.price !== "") ? "" : "shine"} ${classes.deletePrice} ${
                      classes.dis
                    }`}
                    // component="p"
                  >
